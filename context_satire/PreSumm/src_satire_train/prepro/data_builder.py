...
# def load_xml_zach(p):
# [p.text.lower().split() for p in list(doc_node.iter('p'))]
...
def tokenize(args):
    ...
    command = ['java', '-cp','C:/Users/zach_surf/Desktop/stanford-corenlp-full-2018-10-05/*','edu.stanford.nlp.pipeline.StanfordCoreNLP', '-annotators', 'tokenize,ssplit',
...
def format_to_bert_zach(args):
    # if (args.dataset != ''):
    #     datasets = [args.dataset]
    # else:
    datasets = ['training','testing'] #['train', 'valid', 'test']
    for corpus_type in datasets:
        a_lst = []
        for json_f in glob.glob(pjoin(args.raw_path, corpus_type+".json")): #glob.glob(pjoin(args.raw_path, '*' + corpus_type + '.*.json')):
            real_name = json_f.split('/')[-1]
            a_lst.append((corpus_type, json_f, args, pjoin(args.save_path, real_name.replace('json', 'bert.pt'))))
        # print(a_lst)
        # pool = Pool(args.n_cpus)
