...
#from models.top_k_predictor import build_predictor
from models.predictor import build_predictor #_prompted import build_predictor
#from models.predictor_baseline import build_predictor
...
def vis_emb_abs(args):
    ...
    model = AbsSummarizer(args, device, checkpoint)
    import pickle
    pickle.dump([(name,param.cpu().data.numpy()) for name,param in model.named_parameters()], open( "sat_use_param.p", "wb" ) )
 #   exit()
 ...
 #        print(mask_src)
         ...
         embed_data[i] = {'src':src.cpu().data.numpy(),'src_features':src_features.cpu().data.numpy() }
      #   dec_states = self.model.decoder.init_decoder_state(src, src_features, with_cache=True)

  #       doc_embed = src_features.cpu().data.numpy() #torch.sum(src_features,axis=1).view((768)).cpu().data.numpy()
...
#for step in range(max_length):
  ...
  #       doc_embed = src_features.cpu().data.numpy() #torch.sum(src_features,axis=1).view((768)).cpu().data.numpy()
   #      embeddings.append(doc_embed)
    #import numpy as np
    import pickle
    pickle.dump(embed_data, open( "sat_use_model_enc.p", "wb" ) )
 #   embeddings = torch.stack(embeddings)
 #   from sklearn.manifold import TSNE
  #  import matplotlib.pyplot as plt
   # np.save("sat002_words.npy",embeddings)
   # X = embeddings
   # X_embedded = TSNE(n_components=2).fit_transform(X)
   # plt.scatter(X_embedded[:,0],X_embedded[:,1])
   # plt.savefig("demo_embs_orig.png")
